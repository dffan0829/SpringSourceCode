...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	//spring 容器的刷新 重要的核心流程
	@Override
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			prepareRefresh();
			
			//获取创建的BeanFactory工厂  bean.xml 里配置的bean 此时已经被创建
			...
			try {
				//当前为空方法 子类可以重写这个方法 在BeanFactory创建后进行一些个性化的设置
		...
		if (beanFactory.containsLocalBean(MESSAGE_SOURCE_BEAN_NAME)) {
			...
			if (this.parent != null && this.messageSource instanceof HierarchicalMessageSource) {
				...
				if (hms.getParentMessageSource() == null) {
					// Only set parent context as parent MessageSource if no
	...
	protected void registerListeners() {
		...
		// Publish early application events now that we finally have a
		// multicaster...
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		//注册EmbeddedValueResolver组件 解析spring表达式语言
	...
	protected void doClose() {
		if (this.active.get() && this.closed.compareAndSet(false, true)) {
			...
			// Stop all Lifecycle beans, to avoid delays during individual
			// destruction.
			try {
